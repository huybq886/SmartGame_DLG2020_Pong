...
model.add(Dense(units=400, activation='relu', kernel_initializer='glorot_uniform'))
# output layer
...
if (resume and os.path.isfile('SmartPong_weights.h5')):
    print("loading previous weights")
    model.load_weights('SmartPong_weights.h5')


with open('convo1.csv', 'w', newline='') as file:
...
while (True):
    ...
    x = np.expand_dims((cur_input - prev_input), axis=2) if prev_input is not None else np.zeros((80, 80, 1))
    ...
    if done:
        ...
        # training
        model.fit(np.array(xs), np.vstack(ys), verbose=0, sample_weight=discount_rewards(rewards, gamma))
        ...
        # save rewards
        file = open('convo1.csv', 'a', newline='')
        writer = csv.DictWriter(file, fieldnames=fieldnames)
